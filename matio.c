...
int writeDoubleMatrix(char *name, double *data, int m, int n)
{
	...
	numArray.name.dataSize = strlen(name);
	numArray.name.data = name;
	...
	fwrite(((NumericArray*)(matlabArray.data))->name.data, 1, 8, matFile);
...
int main(int argc, char *argv[])
{
	...
	writeDoubleMatrix("x___data", values, 1, 9);
	writeDoubleMatrix("y___data", values, 9, 1);
	writeDoubleMatrix("z___data", values, 3, 3);
	fclose(matFile);
